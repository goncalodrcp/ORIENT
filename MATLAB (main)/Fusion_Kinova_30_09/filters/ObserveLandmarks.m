...
image = rgb2gray(imread(fileImages{IdxImage}));  
...
for i = 1:length(validityMain)
    if validityMain(i) == 1
        ...
        pixelEst = pointsEst(1:2)/pointsEst(3); %Original equation
%         disp('Tracked point');
%         disp(pointsMain(i,:)');
%         disp('Pixel');
%         disp(pixelEst);
        pixelError = norm(pixelEst-pointsMain(i,:)');
        disp('Pixel error (estimate-tracked point):');
        disp(pixelError);
        if norm(pixelEst-pointsMain(i,:)') < EcartPixelMax %reject outlier
            ...
            validityMain(i) = 0;
            %disp(norm(pixelEst-pointsMain(i,:)'));
        end
    end
end
