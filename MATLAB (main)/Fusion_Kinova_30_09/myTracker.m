...
for i = 2:300
    ...
    trackLostFlag = isempty(validPoints);
    
    if(trackLostFlag) 
        orb_slam = trackerReset(fileImages,i,ParamFilter,cameraParams,orb_slam);
        sprintf('Tracking lost at image:%d \n',i)
        disp('Reseting trackers');
        continue;
    elseif(length(validPoints) < 2)
        orb_slam = trackerReset(fileImages,i,ParamFilter,cameraParams,orb_slam);
        sprintf('Too few tracked points at image:%d \n',i)
        disp('Reseting trackers');
        continue;
    end
...
function [newTrackers] = trackerReset(fileImages,i,ParamFilter,cameraParams,orb_slam)

    num_tracks = 200; %it was 100
    
    image = im2uint8(fileImages{i}); %start at image i where the track was lost
    image = undistortImage(image,cameraParams);
    new_points = detectMinEigenFeatures(image); %detect new interest points
    new_points = selectUniform(new_points,num_tracks+30,size(image)); %select new points
    for i=1:num_tracks
        newTrackers.myTracks(i) = pointTrack(1, new_points(i).Location);
    end
    
    %tracker bis new points
    newTrackers.trackerBis = orb_slam.trackerBis;
    setPoints(newTrackers.trackerBis,new_points(1:num_tracks).Location);
    
    %main tracker points
    new_points = selectUniform(new_points,30,size(image)); %select new points
    newTrackers.trackerMain = orb_slam.trackerMain;
    setPoints(newTrackers.trackerMain,new_points.Location);
    %initHelper = init_points.Location;

    newTrackers.PosAmers = zeros(3,30);

    for i=1:30
        newTrackers.PosAmers(1,i) = (new_points.Location(i,1) - ParamFilter.Pi(1,3)) / ParamFilter.Pi(1,1);
        newTrackers.PosAmers(2,i) = (new_points.Location(i,2) - ParamFilter.Pi(2,3)) / ParamFilter.Pi(2,2);
        newTrackers.PosAmers(3,i) = 1;
    end
